...
def y_annotations_to_point_list(y_pred, threshold):
    """ Convert raw prediction to a predicted point list: classification of pixel as containing dot > threshold
    ...
    for ind in range(np.shape(y_pred)[1]):
        ...
        dot_centers.append([[y_ind+delta_y[y_ind, x_ind],x_ind+delta_x[y_ind, x_ind]] for y_ind, x_ind in dot_pixel_inds])
    
    return np.array(dot_centers)

def y_annotations_to_point_list_restrictive(y_pred, threshold):
    """ Convert raw prediction to a predicted point list: classification of pixel as containing dot > threshold 
    AND center regression is contained in the pixel
...
def y_annotations_to_point_list_max(y_pred, threshold=0.95, min_distance=2):
    """ Convert raw prediction to a predicted point list using PLM to determine local maxima in classification prediction image
