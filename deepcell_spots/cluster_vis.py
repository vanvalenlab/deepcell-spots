...
def label_graph_ann(G, coords, exclude_last=False):
    ...
    ------------
    G : networkx graph
        Graph with edges indicating clusters of points assumed to be derived from the same ground truth detection
    coords : matrix
...
def label_graph_gt(G, detection_data, gt):
    ...
    detection_data : matrix
        Matrix with dimensions (number of clusters) x (number of algorithms) with value of 1 if an algorithm detected 
        the cluster and 0 if it did not
    gt : array
        Array with length (number of cluster) with value of 1 if cluster is a true positive detection and 0 if it is a
        false positive
...
def label_graph_prob(G, detection_data, p_matrix):
    ...
    detection_data : matrix
        ...
        the cluster and 0 if it did not
    p_matrix : matrix
        Matrix with dimensions (number of clusters) x 2 where first column is the probability that a cluster is a true positive
        and second column is the probability that it is a false positive
    Returns:
    ----------
    G_new : networkx graph
        Labeled graph
