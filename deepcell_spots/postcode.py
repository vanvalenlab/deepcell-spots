...
def map_states(data, N, D, C, R, K, codes, batch_size=None, temperature=0):
    inferred_model = infer_discrete(poutine.replay(model_constrained_tensor, trace=poutine.trace(auto_guide_constrained_tensor).get_trace(data, N, D, C, R, K, codes)), temperature=temperature, first_available_dim=-2)  # avoid conflict with data plate
